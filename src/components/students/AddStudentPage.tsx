...
export const AddStudentPage: React.FC<AddStudentPageProps> = ({ onNavigateTab }) => {
  ...
  const validate = (): boolean => {
    ...
    } else if (existingStudents.some(s => s.register_number.toUpperCase() === regNum.toUpperCase())) {
    ...
    } else if (existingStudents.some(s => s.email.toLowerCase() === emailVal.toLowerCase())) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMessage(`Student "${created.name}" (${created.register_number}) has been created successfully!`);
      setTimeout(() => {
        onNavigateTab('students');
      }, 1200);
    } catch (err: any) {
      // Server-side validation error displayed clearly
      const msg = err.message || 'Failed to create student.';
      setErrorMessage(msg);

      // Map backend validation to specific field error if possible
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white tracking-tight">Add New Student</h1>
            <p className="text-xs sm:text-sm text-slate-400">
              Create a new student enrollment record with full client and server validation.
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6" noValidate>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div className="space-y-1.5">
              ...
              <input
                id="input-student-register-number"
                type="text"
                placeholder="e.g. 717621CS105"
              ...
              {fieldErrors.register_number && (
                <p id="error-student-register-number" className="text-xs text-rose-400 font-medium">{fieldErrors.register_number}</p>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              />
              {fieldErrors.name && (
                <p id="error-student-name" className="text-xs text-rose-400 font-medium">{fieldErrors.name}</p>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              />
              {fieldErrors.email && (
                <p id="error-student-email" className="text-xs text-rose-400 font-medium">{fieldErrors.email}</p>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              />
              {fieldErrors.phone && (
                <p id="error-student-phone" className="text-xs text-rose-400 font-medium">{fieldErrors.phone}</p>
              )}
            </div>
            ...
            <div className="space-y-1.5 sm:col-span-2">
              ...
              {fieldErrors.department && (
                <p id="error-student-department" className="text-xs text-rose-400 font-medium">{fieldErrors.department}</p>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              {fieldErrors.year && (
                <p id="error-student-year" className="text-xs text-rose-400 font-medium">{fieldErrors.year}</p>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              />
              {fieldErrors.section && (
                <p id="error-student-section" className="text-xs text-rose-400 font-medium">{fieldErrors.section}</p>
              )}
            </div>
            ...
            <div className="space-y-1.5 sm:col-span-2">
              ...
              <input
                id="input-student-admission-date"
                type="date"
