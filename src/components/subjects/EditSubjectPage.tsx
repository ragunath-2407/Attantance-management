...
export const EditSubjectPage: React.FC<EditSubjectPageProps> = ({
  subjectId,
  onNavigateTab,
}) => {
  ...
  useEffect(() => {
    const loadSubject = async () => {
      ...
      try {
        ...
        setOtherSubjects(allSubjects.filter(s => s.id !== subjectId));
        setFormData({
        ...
        setErrorMessage(err.message || `Failed to load subject #${subjectId}`);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const validate = (): boolean => {
    ...
    } else if (otherSubjects.some(s => s.subject_code.toUpperCase() === codeVal.toUpperCase())) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMessage(`Subject "${updated.subject_name}" updated successfully!`);
      setTimeout(() => {
        onNavigateTab('subjects');
      }, 1200);
    } catch (err: any) {
      const msg = err.message || 'Failed to update subject course.';
      setErrorMessage(msg);
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white tracking-tight">Edit Subject Course</h1>
            <p className="text-xs sm:text-sm text-slate-400">
              Update course configuration with full client and server validation.
            </p>
          </div>
        </div>
      ...
      <div className="p-6 sm:p-8 rounded-2xl bg-slate-900 border border-slate-800 shadow-sm">
        {isLoading ? (
          <div className="p-12 text-center text-slate-400 flex flex-col items-center justify-center space-y-3">
            ...
            <p className="text-xs sm:text-sm">Fetching subject record from database...</p>
          </div>
        ) : (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div className="space-y-1.5">
                ...
                {fieldErrors.subject_code && (
                  <p id="error-edit-subject-code" className="text-xs text-rose-400 font-medium">{fieldErrors.subject_code}</p>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                {fieldErrors.subject_name && (
                  <p id="error-edit-subject-name" className="text-xs text-rose-400 font-medium">{fieldErrors.subject_name}</p>
                )}
              </div>
              ...
              <div className="space-y-1.5 sm:col-span-2">
                ...
                <select
                  id="select-edit-subject-department"
                ...
                {fieldErrors.department && (
                  <p id="error-edit-subject-department" className="text-xs text-rose-400 font-medium">{fieldErrors.department}</p>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                {fieldErrors.year && (
                  <p id="error-edit-subject-year" className="text-xs text-rose-400 font-medium">{fieldErrors.year}</p>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <select
                  id="select-edit-subject-semester"
                ...
                {fieldErrors.semester && (
                  <p id="error-edit-subject-semester" className="text-xs text-rose-400 font-medium">{fieldErrors.semester}</p>
                )}
              </div>
            </div>
